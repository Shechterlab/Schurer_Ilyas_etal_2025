...
convert_and_map_entrez <- function(df, species = "Homo sapiens") {
  ...
  if (!is.null(OrgDb)) {
    ...
    if (!is.null(mapped_genes) && nrow(mapped_genes) > 0) {
      ...
      # Merge and correctly assign ENTREZ IDs
      df <- df %>%
        dplyr::left_join(mapped_genes, by = c("gene_id" = "ENSEMBL")) %>%
        dplyr::mutate(entrezgene_id = as.character(ENTREZID)) %>%
        dplyr::select(-ENTREZID)  # Remove unnecessary columns
  ...
  if (!is.null(mart_dataset)) {
    ...
    if (!is.null(mart)) {
      ...
      if (nrow(mapped_genes) > 0) {
        ...
        df <- df %>%
          dplyr::left_join(mapped_genes, by = c("gene_id" = "ensembl_gene_id")) %>%
          dplyr::mutate(entrezgene_id = as.character(entrezgene_id)) # Ensure proper assignment
  ...
  df <- df %>%
    dplyr::left_join(msigdb_genes, by = c("gene_id" = "gs_name")) %>%
    dplyr::mutate(entrezgene_id = as.character(entrez_gene)) %>%
    dplyr::select(-entrez_gene) # Remove unnecessary column

  # Ensure ENTREZ IDs were assigned correctly
  valid_entrez_count <- sum(!is.na(df$entrezgene_id))
...
# Ensure all datasets have ENTREZ IDs before analysis
data_list <- lapply(data_list, function(df) {
  convert_and_map_entrez(df, species = params$species)
})
